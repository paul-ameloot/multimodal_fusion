...
class ExperimentManager(Node):
    ...
    def __init__(self) -> None:
        ...
        # Publishers
        self.ttsPub   = self.create_publisher(String, '/alexa/tts', queue_size=1)
        self.eventPub = self.create_publisher(String, '/alexa/events', queue_size=1)

        # Enable Obstacle Publisher
        self.enableAllObstaclesPub    = self.create_publisher(Bool, '/manipulator_planner/enable_all_obstacles', queue_size=1)
        self.enableGreenObstaclePub   = self.create_publisher(Bool, '/manipulator_planner/enable_green_obstacle', queue_size=1)
        self.enableRedObstaclePub     = self.create_publisher(Bool, '/manipulator_planner/enable_red_obstacle', queue_size=1)
        self.enableYellowObstaclePub  = self.create_publisher(Bool, '/manipulator_planner/enable_yellow_obstacle', queue_size=1)
        self.enableSpecialObstaclePub = self.create_publisher(Bool, '/manipulator_planner/enable_special_obstacle', queue_size=1)

        # Stop Trajectory Publisher
        self.stopTrajectoryPub = self.create_publisher(Bool, '/trajectory_scaling/stop_trajectory', queue_size=1)
        self.stopSimulationPub = self.create_publisher(Bool,'/simulation_scaling/stop', queue_size=1)

        # Subscribers
        self.create_subscription(FusedCommand, '/multimodal_fusion/fused_command', self.commandCallback)
        self.create_subscription(TrajectoryError, '/trajectory_error', self.trajectoryErrorCallback)

        # Set Parameters Service
        self.set_planning_parameters_service = self.create_client(PlanningParameters, '/manipulator_planner/set_planning_parameters')

        # Load Parameters
        self.gripper_enabled = self.get_parameter('experiment.gripper_enabled').get_parameter_value().bool_value
    ...
    def trajectoryErrorCallback(self, data:TrajectoryError):
        ...
        self.get_logger().warn(f"Received Error: {data.error} | {data.info}")
    ...
    def handover(self, object_name, pick_position, place_position) -> bool:
        ...
        self.get_logger().info('Sleep for 1 second')
        rclpy.sleep(1)
    ...
    def placeObject(self, place_area, object_name) -> bool:
        ...
        for area in area_list:
            ...
            if area.name == place_area:
                ...
                self.get_logger().info(f'Place Area Found: {place_area}')
                area_found = True
                break
        ...
        # Area Not Found -> Return
            self.get_logger().error(f'Place Object Function | Place Area Not Found: {place_area} | Stop Experiment')
        else:  self.get_logger().warn(f'Place Object Function | Place Area Found: {place_area}')


        if object_name == 'special_block':

            # Forward + Inverse Kinematic -> Increase Place z + 5cm
            self.get_logger().info('Forward Kinematic + Inverse Kinematic -> Increase Place z + 5cm')
            place_position_cartesian: Pose() = self.robot.FK(place_position)
    ...
    def errorHandling(self, handover_error, object_name, goal_position=None) -> bool:
        ...
        if handover_error in [OPEN_GRIPPER_ERROR, CLOSE_GRIPPER_ERROR, OPEN_GRIPPER_AFTER_ERROR]:
            ...
            self.get_logger().error('ERROR: An exception occurred during Gripper Movement -> Stop Experiment')
        ...
        # Pick Object Movement Error -> Stop Handover
            ...
            self.get_logger().error('ERROR: An exception occurred during Pick Object Movement -> Stop Experiment')
            self.error_stop = True
            return False
        ...
        # Move Home Error -> Stop Handover
            ...
            self.get_logger().error('ERROR: An exception occurred during Move to Home -> Stop Experiment')
            self.error_stop = True
            return False
        ...
        # Place Object Movement Error -> Check Error Type
            ...
            while self.received_error is None and not self.node.is_shutdown(): self.get_logger().info_throttle(5, 'Waiting for Error Message')
            ...
            if self.received_error == OBSTACLE_DETECTED_ERROR:
                ...
                self.get_logger().warn('ERROR: Obstacle Detected')

                # Wait for Error Handling Command
                while self.error_handling_command is None and not rclpy.ok():
                    self.get_logger().info_throttle(5, 'Waiting for Error Handling Command')

                # Object Moved -> Move to Place -> Restart Handover
                if self.error_handling_command.fused_command == OBJECT_MOVED:

                    self.get_logger().info('Object Moved Command -> Retry Place Object')
                    ...
                    if self.robot.move_joint(goal_position): return_flag = True
                        ...
                        self.get_logger().error('ERROR: An exception occurred during Object Moved Error Handling -> Stop Experiment')
                        self.error_stop = True
                        return False
                ...
                # Put Object in Place Area -> Place Object -> Stop Handover
                    ...
                    self.get_logger().info('Place Object Area Command -> Call Place Object Function')
                    ...
                    if not self.placeObject(place_area=self.error_handling_command.area, object_name=object_name):
                        ...
                        # Negative Error Handling -> Stop Handover
                        self.get_logger().error('ERROR: An exception occurred during Place Object in Area Error Handling -> Stop Experiment')
                        self.error_stop = True
                    ...
                    # Stop Handover
                    self.get_logger().warn('WARN: Wait for Command Error Handling Received')
            ...
            # Move to User Error -> Stop Handover
                ...
                self.eventPub.publish(error_msg)
                self.get_logger().warn('ERROR: Move to User Error')
                ...
                # Wait for Error Handling Command
                while self.error_handling_command is None and rclpy.ok():
                    self.get_logger().info_throttle(5, 'Waiting for Error Handling Command')

                # User Moved -> Move to Place -> Restart Handover
                if self.error_handling_command.fused_command == USER_MOVED:

                    self.get_logger().info('User Moved Command -> Retry Place Object')
                    ...
                    if self.robot.move_joint(goal_position): return_flag = True
                        ...
                        # Negative Error Handling -> Stop Handover
                        self.get_logger().error('ERROR: An exception occurred during User Moved Error Handling -> Stop Experiment')
                        self.error_stop = True
                        return False
                ...
                # Wait Time -> Move to Place -> Restart Handover
                    ...
                    # Wait Time
                    self.get_logger().info('Wait Time')
                    print(type(self.error_handling_command.wait_time))
                    rclpy.sleep(30 if self.error_handling_command.wait_time is not int else self.error_handling_command.wait_time)
                    ...
                    if self.robot.move_joint(goal_position): return_flag = True
                        ...
                        # Negative Error Handling -> Stop Handover
                        self.get_logger().error('ERROR: An exception occurred during Wait Time Error Handling -> Stop Experiment')
                        self.error_stop = True
                        return False
            ...
            self.get_logger().error(f'ERROR: An exception occurred with Untracked Error {handover_error} -> Stop Experiment')
            self.error_stop = True
            return False
    ...
    def collaborativeReleaseRoutine(self):
        ...
        self.eventPub.publish(error_msg)
        self.get_logger().warn('ERROR: Help Special Block Error')
        ...
        if self.error_handling_command.fused_command == CAN_GO:
            ...
            # Other Errors -> Stop Handover
            self.get_logger().error(f'ERROR: An exception occurred with Incorrect Command {self.error_handling_command.fused_command} -> Stop Experiment')
            self.error_stop = True
            return False
    ...
    def enableObstacle(self, obstacle_name, enable=True):
        ...
        if obstacle_name == 'green_block':   self.enableGreenObstaclePub.publish(msg);  self.get_logger().warn('Enable Green Obstacle')
        ...
        msg.data = False
        if obstacle_name == 'yellow_block': self.enableAllObstaclesPub.publish(msg); self.get_logger().warn('Disable All Obstacles')
    ...
    def run(self):
        ...
        for object in object_list:
            ...
            self.get_logger().warn(f"Handover Object: {object.name}")
            ...
            if self.wait_for_command:
                ...
                # Wait for Command
                while self.error_handling_command is None and rclpy.ok():
                    self.get_logger().info_throttle(5, 'Waiting for Command')
                ...
                if self.error_handling_command.fused_command in [PLACE_OBJECT_GIVEN_AREA, PLACE_OBJECT_GIVEN_AREA_POINT_AT]:
                    ...
                    # Stop Handover
                    self.get_logger().error('ERROR: Wrong Command Received -> Stop Experiment')
                    self.error_stop = True
                    break
        ...
        msg = Bool()
        msg.data = False
        self.get_logger().warn('Disable All Obstacles')
