...
class Fusion(Node):
    ...
    def __init__(self):
        ...
        # Publishers
        self.fusionPub = self.create_publisher(FusedCommand, '/multimodal_fusion/fused_command', queue_size=1)

        # Subscribers
        self.voice_sub = self.node.create_subscription(VoiceCommand, '/multimodal_fusion/voice_command', self.voiceCallback, 10)
        self.gesture_sub = self.node.create_subscription(Int32MultiArray, '/multimodal_fusion/gesture_command', self.gestureCallback, 10)
        self.area_sub = self.node.create_subscription(Int32MultiArray, '/multimodal_fusion/point_area', self.areaCallback, 10)
        ...
        self.get_logger().warn('Multimodal Fusion Initialized')
    ...
    def voiceCallback(self, data:VoiceCommand):
        ...
        self.get_logger().info(f'Received Voice Command: {self.voice_msg} | {data.info}')
    ...
    def gestureCallback(self, data:Int32MultiArray):
        ...
        if self.recognition_thread_active == False:
            ...
            self.get_logger().info('Gesture Recognition Started')
